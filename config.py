...
PERSONA_ANALYSIS_PROMPT = """사용자의 소비 패턴을 분석하여 카드 추천에 중요한 항목을 중요도 순서로 정리하세요.

사용자 정보:
{persona}

아래 형식으로만 출력하세요 (개인 정보·국적·이름 제외, 소비 패턴만):

**소비 패턴 요약**
1순위: [소비 항목] — [월 지출액] / [필요 혜택]
2순위: [소비 항목] — [월 지출액] / [필요 혜택]
(중요 항목이 있는 만큼 작성)

**카드 검색 키워드**
[혜택1], [혜택2], [혜택3], ...

중요도 기준: 월 지출액이 크거나 자주 언급된 항목을 높은 순위로 배치하세요."""
